...
def criarScript(nodes):
    try:
        dispo = nodes
        dados = f'quantidade={len(dispo)}\r'
        index = 0
        for x in dispo:
            info = NodeSetup.objects.get(node=x)
            ...
            dados = f'{dados}limiarPico[{index}]={int(info.limiarPico)}\r'
            dados = f'{dados}limiarMedia[{index}]={int(info.limiarMedia)}\r'
            index = index+1
        VTFile = open("VT.sb",'r')
        ...
        print(f"falha ao criar Script: {str(EX)}")
    ...
    while controle:
        try:
            ...
            try:
                ...
                v = len(dxm.readReg(1,1,slaveID=199))
                ip.online= True
                ip.save()
            except Exception as EX:
                ip.online= False
                ip.save()
                print(f"erro ao ler se esta ONLINE{str(EX)}")
                sleep(5)
            ...
            if ip.online:    
                ...
                try:
                    ...
                    for n in nodes:
                        dxm = Protocolo(ip.port)
                        info = NodeSetup.objects.get(node=n)
                    ...
                    print(f"Falha ao ler regs do DXM{str(EX)}")
                # realiza o log
                try:
                    if dxm.fileExist("SbFile1.dat"):
                        sleep(2)
                    ...
                    print(f"Falha ao processar log{str(EX)}")
                ...
                try:
                        ...
                        for n in range(len(nodes)):
                            if int(dxm.readReg(n*50+10,1,slaveID=199)[0]) == 0:
                                ...
                                if not corrompido:
                                    ...
                                    apren.limiarMedia = int(obj[0]['limiarMedia'])
                                    apren.limiarPico = int(obj[0]['limiarPico'])
                                    apren.save()
                                    sleep(1)
                                    contagem = 0
                                    for x in obj:
                    ...
                    print(f"Falha ao processar o Aprendizado{str(EX)}")

        except Exception as EX:
            sleep(5)
            print(f"falha geral {str(EX)}")
